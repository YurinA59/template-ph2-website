...
// $stmt = $dbh->query($sql); // SQLを実行して、実行した結果を入れた箱　SQL を MySQL に送って実行する
// $questions = $stmt->fetchAll(PDO::FETCH_ASSOC); // 結果を配列で取り出して取得した質問データ（配列）を入れた箱
// ↓↓↓代入するとこれじゃん

$questions = $dbh->query("SELECT * FROM questions")->fetchAll(PDO::FETCH_ASSOC);
// questions テーブルの全データを取り出して、
// 配列（1つの変数は「1つの箱」配列は「仕切りがたくさんあるお弁当箱」）として受け取る
$choices = $dbh->query("SELECT * FROM choices")->fetchAll(PDO::FETCH_ASSOC);

foreach ($questions as $qKey => $question) {
//$questionsっていう配列があって、 一つ一つ問題に対して処理しますよ→foreach
    $question["choices"] = [];
// 問1の選択肢全部的なノリ。
...
// choicesの中には選択肢がいっぱい！！だから問１の選択肢なのか問２の選択肢なのかやろう！
    }
